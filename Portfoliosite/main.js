document.addEventListener('DOMContentLoaded', () => {
  // KSA data
  const K = [
    {label:'서비스기획', g:'plan'}, {label:'UX/UI 설계', g:'plan'},
    {label:'데이터분석', g:'data'}, {label:'Power BI', g:'data'}, {label:'GA4', g:'data'}
  ];
  const S = [
    {label:'문제정의', g:'plan'}, {label:'가설설정', g:'plan'},
    {label:'리서치설계', g:'plan'}, {label:'유저인터뷰', g:'plan'}
  ];
  const A = [
    {label:'논리적사고', g:'plan'}, {label:'분석적사고', g:'data'}, {label:'사용자중심사고', g:'ux'}
  ];
  ...
  function renderKSA(){
    ...
    groups.forEach(g=>{
      ...
      g.items.forEach(x=>{
        if(ksaFilter==='all' || x.g===ksaFilter){
  ...
  if(grid){
    projects.forEach(p => {
      ...
      const imgHtml = p.image ? `<img src="${p.image}" alt="${p.title}" loading="lazy" class="w-full h-44 object-cover"/>` : '';
      el.innerHTML = `
